...
for i in "${!prediction_horizon_list[@]}"; do
	...
	# Run the command
	python train_acmpc_multienv_dynamical_system_args.py --seed 0208 --prediction_horizon "$prediction_horizon" --num_optimization_step "$num_optimization_step" --log_name "${name_main}_${name}" --save_name "models/${name_main}/${name}_1"
	...
	# Run the command
	python train_acmpc_multienv_dynamical_system_args.py --seed 0411 --prediction_horizon "$prediction_horizon" --num_optimization_step "$num_optimization_step" --log_name "${name_main}_${name}" --save_name "models/${name_main}/${name}_2"
done
