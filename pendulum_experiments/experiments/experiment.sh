...
for i in "${!prediction_horizon_list[@]}"; do
	...
	# Run the command
	python train_acmpc_multienv_pendulum_args.py --seed 0208 --prediction_horizon "$prediction_horizon" --num_optimization_step "$num_optimization_step" --tb_log_name "${name_main}_${name}" --save_name "models/${name_main}_${name}_1"
	...
	# Run the command
	python train_acmpc_multienv_pendulum_args.py --seed 0411 --prediction_horizon "$prediction_horizon" --num_optimization_step "$num_optimization_step" --tb_log_name "${name_main}_${name}" --save_name "models/${name_main}_${name}_2"
done
