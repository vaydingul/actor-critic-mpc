...
for i in "${!prediction_horizon_list[@]}"; do
	...
	echo "$name"
	# Run the command
	python train_acmpc_multienv_pendulum_args.py --seed 42 --prediction_horizon "$prediction_horizon" --num_optimization_step "$num_optimization_step" --tb_log_folder "$folder/tensorboard_logs/" --tb_log_name "${name_main}_${name}" --save_name "$folder/models/${name_main}_${name}_1"
	python train_acmpc_multienv_pendulum_args.py --seed 2023 --prediction_horizon "$prediction_horizon" --num_optimization_step "$num_optimization_step" --tb_log_folder "$folder/tensorboard_logs/" --tb_log_name "${name_main}_${name}" --save_name "$folder/models/${name_main}_${name}_2"
done
